...
export default function Breadcrumbs(props: BreadcrumbsProps) {
  ...
  React.useEffect(() => {
    ...
    function insertPatientUuid(route) {
      route.url = currentPatientUuid ? route.url.replace(':patientUuid', currentPatientUuid) : route.url;
      return route;
    }
        ...
        breadcrumbs.map((route, index) => {
          return (
            <li className={styles.route} key={index}>
              <Link
                to={route.url}
                className={`${styles.breadcrumb} ${isActiveRoute(index) ? styles.active : styles.inactive}`}>
                {route.name}
              </Link>
              {!isActiveRoute(index) && (
                <div>
