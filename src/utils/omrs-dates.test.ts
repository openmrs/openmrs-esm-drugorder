import { isOmrsDateStrict, toDateObjectStrict, toOmrsDateString } from './omrs-dates';
...
describe('Openmrs Dates', () => {
  it('checks if a string is openmrs date', () => {
    ...
    expect(isOmrsDateStrict(null)).toEqual(false);
    expect(isOmrsDateStrict(undefined)).toEqual(false);
  });
  ...
  it('converts js Date object to omrs date string version', () => {
    ...
    expect(toOmrsDateString(date, true)).toEqual('2018-03-18T21:05:03.999+0000');
  });
});
