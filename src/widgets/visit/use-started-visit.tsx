  ...
  useEffect(() => {
    const getRequiredVisit = (visits: Array<Visit>) => {
      ...
      const todayVisitsForPassedType = visits.filter(visit => {
        return visit.visitType.uuid === props.visitTypeUuid && dayjs(visit.startDatetime).isSame(today, 'date');
      });
    ...
    patientSub = getCurrentPatientUuid().subscribe(patientUuid => {
      visitSub = getVisitsForPatient(patientUuid, new AbortController()).subscribe(
        visits => {
