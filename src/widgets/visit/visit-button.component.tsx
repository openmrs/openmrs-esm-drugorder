...
import { getStartedVisit, visitItem, visitMode, visitStatus } from './visit-utils';
import dayjs from 'dayjs';
import { isEmpty } from 'lodash-es';
import { newModalItem } from './visit-dialog.resource';
import { newWorkspaceItem, useCurrentPatient } from '@openmrs/esm-api';
import { updateVisit, UpdateVisitPayload, getVisitsForPatient } from './visit.resource';
      ...
      selectedVisit && (
        <div className={styles.editContainer}>
          ...
          <svg
            ...
            onClick={() => {
              newModalItem({
                component: <CloseActiveVisitConfirmation currentVisit={getStartedVisit.value} />,
                name: 'Cancel Visit',
                props: null,
              });
            }}>
...
const CloseActiveVisitConfirmation: React.FC<EndVisitProps> = ({ currentVisit }) => {
  return (
    <div className={styles.visitPromptContainer}>
      <h2>Are you sure to close this visit</h2>
      <p>
        Visit Type : {currentVisit.visitData.visitType.display} Location: {currentVisit.visitData?.location?.display}{' '}
        Start Date: {dayjs(currentVisit.visitData.startDatetime).format('DD-MMM-YYYY')}
      </p>
      <div className={styles.visitPromptButtonsContainer}>
        <button
          className={`omrs-btn omrs-outlined-action`}
...
export const EndVisit: React.FC<EndVisitProps> = ({ currentVisit }) => {
  return (
    <div className={styles.visitPromptContainer}>
      <h2>
        <Trans i18nKey="End Visit Prompt Message">Are you sure you wish to end this visit?</Trans>
      </h2>
      <p>
        Visit Type : {currentVisit.visitData.visitType.display} Location : {currentVisit.visitData.location.display}{' '}
        Start Date : {dayjs(currentVisit.visitData.startDatetime).format('DD-MMM-YYYY')}
