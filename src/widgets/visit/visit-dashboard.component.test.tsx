...
import { render, screen, fireEvent, RenderResult, findByRole, findByText } from '@testing-library/react';
import { getCurrentPatientUuid, openmrsObservableFetch, useCurrentPatient, openmrsFetch } from '@openmrs/esm-api';
import { of } from 'rxjs';
import { mockVisitTypesDataResponse, mockVisits } from '../../../__mocks__/visits.mock';
...
describe('VisitDashboard', () => {
  ...
  it('should display new visit and edit visit buttons', async () => {
    expect(await screen.findByRole('button', { name: /New Visit/i })).toBeInTheDocument();
    expect(await screen.findByRole('button', { name: /Edit Visit/i })).toBeInTheDocument();
  });
