...
import { useCurrentPatient, openmrsFetch, openmrsObservableFetch } from '@openmrs/esm-api';
...
describe('MedicationOrder', () => {
  ...
  it('render without dying', async () => {
    ...
    mockgetPatientEncounterID.mockReturnValue(Promise.resolve(mockPatientEncounterIDResults));
    mockgetPatientDrugOrderDetails.mockReturnValue(Promise.resolve({}));
    mockgetDurationUnits.mockReturnValue(Promise.resolve(mockDurationUnitsResults));
    const wrapper = render(
      <BrowserRouter>
        <MedicationOrder drugName={''} editProperty={[]} orderEdit={{ orderEdit: false, order: null }} />
      </BrowserRouter>,
    );
